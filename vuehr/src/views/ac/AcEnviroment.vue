                ...
                <el-form :model="row" :rules="rules" ref="rowForm">
                    <el-row>
                        <el-form-item label="环境名称:" prop="envName">
                            <el-input size="mini" style="width: 200px" prefix-icon="el-icon-edit" v-model="row.envName"
                                      placeholder="请输入环境名称"></el-input>
                        </el-form-item>
                    </el-row>
                    <el-row>
                        <el-form-item label="描述信息:" prop="envDesc">
                            <el-input size="mini" style="width: 200px" prefix-icon="el-icon-edit" v-model="row.envDesc"
        ...
        methods: {
            ...
            deleteRow(data) {
                this.$confirm('此操作将永久删除【' + data.name + '】, 是否继续?', '提示', {
                    ...
                    this.deleteRequest("/ac/envrionment/" + data.id).then(resp => {
                        if (resp) {
                            this.initRows();
                        }
                    })
            ...
            doAddRow() {
                if (this.row.id) {
                    this.$refs['rowForm'].validate(valid => {
                        if (valid) {
                            this.putRequest("/ac/envrionment", this.row).then(resp => {
                                if (resp) {
                    ...
                    this.$refs['rowForm'].validate(valid => {
                        if (valid) {
                            this.postRequest("/ac/envrionment", this.row).then(resp => {
                                if (resp) {
                                    this.dialogVisible = false;
                                    this.initRows();
                                }
